...
| B.3 | Endpoint: `POST change-requests/:id/verify` (+ un-verify back to `in_progress`) | **BLOCKED** — needs a Cerbos `verify` action, and `cerbos-catalog-alignment.test.ts` (b) CI-gates any policy action with no catalog entry. See B.3a |
