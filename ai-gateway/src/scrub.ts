// DLP scrubber (mirrors wa-chat-bot/src/scrub.ts VERBATIM — components are separate
// projects, not a shared package). Keep in sync; SCRUB_RULESET_VERSION must match the
// canonical copy (guarded by scrub.test.ts). Redacts before any external egress.
...
/** Fail-closed DLP (D8.2): if scrubbing itself fails, the call is BLOCKED, never passed raw. */
export function dlp(input: string): ScrubResult {
  try {
    return scrub(input);
  } catch (err) {
    throw new Error(`DLP unavailable — egress blocked (fail-closed): ${(err as Error).message}`);
  }
}
