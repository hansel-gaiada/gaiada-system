...
// NOTE: PUT /api/admin/gateway/config does not exist on the backend yet.
// This action is wired up so the UI degrades gracefully — on a 404/405 it
// surfaces a friendly message instead of crashing. Remove this fallback once
// the gateway admin API ships config writes.
export async function updateGatewayConfig(
  ...
  try {
    await platformFetch("/api/admin/gateway/config", userId, {
      method: "PUT",
    ...
    if (e instanceof PlatformError) {
      if (e.status === 404 || e.status === 405) {
        return { ok: false, error: "Saving isn't available yet — the gateway admin API is pending." };
      }
