...
import { groupProjectsByClient, taskDateEnvelope, tallyProjectTasks, targetNote } from "./page-helpers";
...
export default async function DepartmentProjectsPage({ params }: { params: Params }) {
  ...
  const [dept, allProjects, clients] = await Promise.all([
  ...
  // A client has many projects — the table is grouped by client, one section each.
  const groups = groupProjectsByClient(owned, new Map(clients.map((c) => [c.id, c.name])));
