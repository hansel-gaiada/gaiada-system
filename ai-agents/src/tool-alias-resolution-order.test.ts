...
// that way.
//
// 2026-08-08 (P4-J5) — REBASED ONTO A SYNTHETIC ALIAS, deliberately, not onto a real tool name. This
// file originally proved the property against the real `pm.listTasks -> tasks.list` entry. That entry
// was retired (see tool-aliases.ts's header) once `pm.listTasks` became a real, DIFFERENT canonical
// tool in its own right (mcp-hub/src/pm-tools.ts, P4-J1) — pinning an ORDERING test to a name that can
// stop being an alias out from under it is exactly the fragility this rebase removes. The property being
// tested is about `runAgent`'s call order, not about which real near-miss happens to be aliased today,
// so `tool-aliases.ts`'s `__setTestAlias`/`__clearTestAliases` test-only overlay injects a synthetic,
// obviously-fake pair for the duration of each test.
import { describe, it, expect, beforeEach, afterEach } from "vitest";
import { runAgent, ApprovalRequiredError, type AgentDef, type AgentDeps } from "./agent";
import { __setTestAlias, __clearTestAliases } from "./tool-aliases";

const envelope = { provider: "telegram", externalId: "tg:1" };
const ALIAS_FROM = "test.aliasProbe";
const ALIAS_TO = "test.canonicalProbe";

describe("tool-alias resolution order — SECURITY (must run before authorization, never after)", () => {
  beforeEach(() => {
    __setTestAlias(ALIAS_FROM, ALIAS_TO);
  });
  afterEach(() => {
    __clearTestAliases();
  });
  ...
  it("resolves BEFORE the allow-list gate: the model names the alias, only the canonical name is allow-listed, and the canonical tool actually runs — the raw guess never reaches deps.callTool nor the off-list refusal loop", async () => {
    const def: AgentDef = {
      ...
      systemPrompt: "test",
      tools: { [ALIAS_TO]: "read" }, // ONLY the canonical name is declared — NOT the alias
      maxSteps: 4,
      maxToolCalls: 4,
    };
    const calls: string[] = [];
    ...
    const deps: AgentDeps = {
      complete: async () =>
        modelTurn++ === 0 ? `{"tool": "${ALIAS_FROM}", "args": {}}` : `{"final": "1 open task"}`,
      callTool: async (name) => {
        calls.push(name);
    ...
    // If resolution ever moved to AFTER this allow-list lookup, `def.tools[ALIAS_FROM]` would be
  ...
  it("resolves BEFORE the D14-12 impact-reconciliation gate: a registry opinion keyed ONLY by the canonical name still escalates the call to high_write and suspends it, even though the model named the alias — the raw alias name carries NO registry opinion of its own, on purpose", async () => {
    const def: AgentDef = {
      ...
      systemPrompt: "test",
      tools: { [ALIAS_TO]: "read" }, // declared READ — only the registry opinion escalates this
      maxSteps: 4,
      maxToolCalls: 4,
    };
    const calls: string[] = [];
    const deps: AgentDeps = {
      complete: async () => `{"tool": "${ALIAS_FROM}", "args": {}}`, // the ALIAS — never the canonical name
      callTool: async (name) => {
        calls.push(name);
        return "must never run";
      },
      // Keyed ONLY by the canonical name. A resolve-after-authorization bug means runAgent still holds
      // the raw alias name at this call site, this returns undefined, effectiveImpact stays "read",
      // and the escalation below never fires — the exact bypass this test exists to catch.
      getRegistryImpact: (name) => (name === ALIAS_TO ? { write: true, impact: "high" } : undefined),
    };
    const err = await runAgent(def, "list my open tasks", envelope, deps).catch((e) => e);
    expect(err).toBeInstanceOf(ApprovalRequiredError);
    expect((err as ApprovalRequiredError).tool).toBe(ALIAS_TO); // the CANONICAL name, never the alias
