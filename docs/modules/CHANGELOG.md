...
### platform-ui `0.64.0` - the invoice maker/checker gets a UI (2026-09-02) - PROTOTYPED
...
> **⚠ FORMAT CHANGED at `1.0.0-alpha.303` (2026-08-31).** The app version is now SemVer 2.0.0
> (`docs/modules/VERSIONING.md`); rows above this point use the retired
> `Alpha 01.071.NNNNa` format. The counter is continuous across the cutover — legacy `…0301a`,
> then `302` (cut but never tagged), then `1.0.0-alpha.303` as the first SemVer build actually
> shipped — so no number is reused and builds stay comparable. Tags gain a `v` prefix
> (`v1.0.0-alpha.303`); `deploy.yml` accepts both spellings while any legacy-format release could
> still be in flight.
