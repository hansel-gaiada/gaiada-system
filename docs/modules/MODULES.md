...
**`1.0.0-alpha.328`** — see [`VERSIONING.md`](./VERSIONING.md) for the format, and
...
|---|---|---|---|---|
| platform-nest | `0.52.0` | IN PROGRESS | WS1 | 2026-09-05 |
...
## platform-nest — Platform Core · `0.52.0` · IN PROGRESS
...
a new idempotent `seed:monitoring-delivery` script for `monitor_channels`/`monitor_routes`/
`monitor_maintenance`, which were nearly empty despite 18 real monitors and 7615 probe results.
...
## ai-gateway-go — AI Gateway · `0.13.2` · PROTOTYPED
...
## mcp-hub — Access Layer · `0.12.1` · PROTOTYPED
...
## automation (n8n) — Orchestration · `0.4.1` · DEV-VERIFIED
...
## observability — Telemetry · `0.6.2` · DEV-VERIFIED
...
## wa-chat-bot — Messaging Surface · `0.9.2` · PROTOTYPED
...
## ai-agents — Agent Brigade · `0.8.1` · PROTOTYPED
...
## webdev — Delivery Rail · Cockpit · `0.13.0` · IN PROGRESS
...
Phase 1; Phase 3 (external wiring — GitHub App, Drive OAuth, Claude Admin usage) is gated on owner
...
## webdesk — Website Platform · `0.2.0` · DEV-VERIFIED
...
## hr — People · Hiring · Pay · `0.5.0` · IN PROGRESS
...
|---|---|---|
| Schema + RLS (33 new tables) | **DEV-VERIFIED** | migrations applied to a real Postgres; `src/db/hr-full-rls.test.ts` proves FORCE RLS, a byte-identical third-wall predicate and zero-rows-without-scope on all 33, plus 7 schema invariants — 20 assertions, **0 skips** |
...
- **A · time and lifecycle** — holiday calendars (incl. Indonesian *cuti bersama*, which is not
  ...
  previous fact.
- **B · recruitment (ATS)** — requisitions, a candidate pool kept deliberately SEPARATE from
...
- **C · compensation and benefits** — pay grades, effective-dated compensation (a GiST exclusion
...
- **D · payroll and separation** — statutory parameter sets, payroll runs, frozen payslips with
  itemized lines, per-period inputs, and separations computing the three Indonesian statutory
  components separately.

### ⚠  The statutory gate, and why payroll exists here at all

`employee-portal-foundation.md` §6 assigned the payroll ENGINE elsewhere and gated it on "statutory
...
**Non-resident withholding (PPh 26) is not implemented** — the engine throws rather than producing a
...
## search-marketing — SEO · SEM · GEO · `0.5.2` · DEV-VERIFIED (schema/RLS/Cerbos layer; the site-audit capability itself is PLANNED)
...
(`social-content-brief-automation.test.ts`, 6/6). A single global principal holding memberships across
every opted-in tenant would be a candidate, via WS8's own tenant-set-wide + `acl='{}'` `/search`
predicate (read directly in `ai-agents/src/knowledge/store.ts` — no per-call `tenantId` narrows it),
to retrieve every OTHER opted-in tenant's entire internal ERP corpus on any one tenant's sweep call —
exactly the "automation identity that can read every tenant's corpus is worse than no sweep" failure
this follow-up was told to avoid. New `src/seed/social-content-brief-automation.ts` (mint/lookup,
least-privilege: no role, no Cerbos grant — this identity never passes through Cerbos, only WS8's OBO
principal lookup) + new `src/modules/social/content-brief-job.ts` (the scheduled sweep itself, dark by
default via `config.social.contentBrief.weeklySweep`, wired in `main.ts`). Opt-in lives on
`social_engagements.tool_scope.ai.autoWeeklyBrief` (additive jsonb, no migration), absent-defaults-
FALSE (the opposite polarity from `ai.drafting`'s absent-defaults-true, since an unattended weekly
draft spending gateway calls needs an explicit opt-in). The sweep LOOKS UP, never mints, the
per-tenant principal — an opted-in tenant with none provisioned is counted
...
**0.5.19–0.5.22 (2026-08-23) — four follow-ups the seats had named rather than silently absorbed.**
Full narrative per version in [`CHANGELOG.md`](./CHANGELOG.md); in short: `0.5.19` a retracted client
review ask is now told to the client (`social.client_review.withdrawn` had no registered handler, so a
withdrawn ask left the client holding a bell entry aimed at a row they could no longer see);
`0.5.20` the AI report narrative can no longer state an invented number (`findUngroundedNumbers`
traces every digit-run to a grounding fact and REJECTS the draft otherwise — the prior reasoning,
"nothing can strip a hallucinated number out of prose", was true but answered a different question:
prose cannot be repaired, yet it can be declined); `0.5.21` a sustained inbox spike is announced once
rather than every tick, using the `outbox_events` log itself as the dedup state rather than a second
store of "did we already say this"; `0.5.22` the metrics pull is gated from `config.social.metricsPull`
like every other job in the module. All four proven red-then-green.
...
**Future plans — RE-PLANNED 2026-08-12, see the addendum
[`../blueprints/smm-design-addendum-2026-08-12.md`](../blueprints/smm-design-addendum-2026-08-12.md)**
(binding over design §12). The base plan was written against the 2026-07-23 platform; six assumptions
...
binding per capability; there is **no image-generation backend** — the gateway has no generative endpoint and
`render-gateway-go` is `0.0.0`; the live client portal makes client post-approval a real surface; migrations
rebase `0034+` → `0105+`). **30 tickets P0–P4 + 3 decision-gated:** P0 substrate/IAM/containment (incl. new
SMM-30 IAM registration + `smm_manager`/`smm_staff` roles) → P1 publish loop on own accounts ($0, publish
registered as an executable approval; X ships disabled so the path carries no money) → P2 inbox **+ client
approval via the portal** (new SMM-31/32, plain-tenant-wall table per 0088's lesson) → P3 AI copy +
analytics + reports (attach-only assets) → P4 agents + assistant. Decision-gated: SMM-28 Mixpost fallback,
...
## reports — Work Tracker · Reports · Appraisal · `0.3.2` · PROTOTYPED
