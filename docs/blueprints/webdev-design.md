...
| OQ-4 | ~~**Rate card**: who owns/maintains it, currency (IDR vs USD), and the approval threshold above which an estimate on a change request requires a client sign~~ **CLOSED-BY-DEFAULT 2026-08-07** — owner authorized proceeding on the stated defaults to unblock P5 rather than answering each question individually. **Adopted: IDR · PM-owned · client-sign threshold Rp 5,000,000.** The threshold is a **money-policy placeholder the owner has not stated a number for** — it is recorded here so it is visible and correctable, and the estimate-helper ticket MUST surface it as configuration (not a constant) so changing it is a config edit, never a migration | ~~Estimate helper (P5)~~ **no longer blocking** | as adopted |
...
| D-2a | **AMENDED + OWNER-RATIFIED 2026-08-07 — `webdev_change_requests` takes the plain CORE tenant wall, NOT the third wall.** D-2's "new `webdev_*` tables take the third wall" rule now carries one stated exception class: **a `webdev_*` table whose PRIMARY writer is the client portal gets the plain tenant wall.** Estimates / rate cards / QA runs are unaffected and stay third-walled | `app_module_allowed(mod)` reads the **request-declared `app.scopes` GUC** (`0028:39-52`), not a company's enabled-module set — so the gate is a two-sided handshake. Portal controllers declare no module scope, therefore a third wall on a portal-reachable table makes **every portal read return zero rows silently**. Two shipped precedents postdate D-2 and rule the same way (`0072:214-215`, `0075:242-244`: *"gating a client's own contract behind a module flag would fail the portal closed for reasons no one would find"*). D-2's own rationale — a wall that breaks siblings structurally is wrong — is what produces this exception; the amendment extends D-2's reasoning rather than overturning it. Design: [maintenance-intake §1.1](../superpowers/plans/2026-08-07-webdev-maintenance-intake-design.md) |
