# HISTORIC — Project Guide as of 2026-07-09 (archived 2026-08-10)

> **Do not treat any status claim below as current.** This is the verbatim former
> `CLAUDE.md` root guide, kept for provenance only. It stopped at `2026-07-09` while the
> program ran on to `Alpha 01.035.0084a`, so its per-component "BUILT / DONE / feature-complete"
> bullets are a month-plus stale (the same failure mode memory `backend-status-2026-07` records).
> Live status lives in `docs/modules/MODULES.md`, `CHANGELOG.md`, `/VERSION`, and the code.
> The durable rules that were mixed in here now live in the rewritten `CLAUDE.md`.
