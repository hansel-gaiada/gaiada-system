...
| R4a | **NDR forgery: suppression of a stream the sender does not own** (MAIL-18 exit gate, 2026-08-08 — HIGH, partially fixed) | Dev fix shipped in `alpha-01.028.0072a`: inbound NDR suppresses ONLY the stream its matched reply token belongs to (never `'*'`), and can NEVER suppress the **auth** stream. Re-driven live: forged 5.1.1 NDR against an auth token ⇒ row `bounced`, **zero** suppression rows; against a notify token ⇒ exactly one `stream='notify'` row; **zero** `stream='*'` rows table-wide. | **The residual.** `classifyNdr()`'s five signals are ALL read from the untrusted message (From, Content-Type, Auto-Submitted, Subject, RFC-3464 body fields), so anyone holding a notify reply token can still forge a bounce and suppress that victim's **notify** mail. Reply tokens ride in the `Reply-To` of every threads-eligible mail the victim receives. In dev the inbound endpoint is behind `MAIL_INBOUND_TOKEN`; **the moment a real Brevo inbound webhook is wired (R3), that wall disappears and this becomes reachable by anyone who can send an email.** | Require corroboration the sender does not control before an inbound NDR suppresses anything: a provider-side bounce EVENT on `/api/mail/webhooks/brevo` (separate `MAIL_WEBHOOK_TOKEN`, provider-authenticated) — treat inbound-parsed content as a *hint* that must be matched by a provider event, or quarantine it for review. Note the delivery-webhook path legitimately still writes `'*'`: it IS provider-authenticated, so "dead everywhere" is a sound conclusion there. | **MUST close BEFORE R3 goes live** — not after. MAIL-18 forged-NDR attack re-run against the real webhook configuration |
