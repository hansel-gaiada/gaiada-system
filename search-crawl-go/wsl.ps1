<#
  ...
  USAGE (from this folder):
    ...
    .\wsl.ps1 run              # go run ./cmd/crawl   (set DATABASE_URL/TENANT_ID/PROPERTY_ID first)
    .\wsl.ps1 test             # go test ./...        (internal/db's live-DB test needs
                                                          TEST_DATABASE_URL set, else it self-skips)
    .\wsl.ps1 vet
